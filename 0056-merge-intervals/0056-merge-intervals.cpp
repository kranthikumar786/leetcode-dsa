    ...
    vector<vector<int>> merge(vector<vector<int>>& intervals) {
        deque<pair<int,int>>de;
         ...
         for(auto interval : intervals) {
               int fLeft = interval[0];
               int fRight = interval[1];
               while(!de.empty() && de.back().second >= fLeft) {
                     fLeft = min(fLeft,de.back().first);
                     fRight = max(fRight, de.back().second);
                     de.pop_back();
               }
            de.push_back({fLeft,fRight});
         }
         vector<vector<int>>ans;
         while(!de.empty()){
             ans.push_back({de.front().first,de.front().second});
             de.pop_front();
         }
       return ans;  
    }
